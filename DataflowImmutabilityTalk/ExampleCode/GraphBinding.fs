...
let bindAction (action: 'T -> unit): Scaffold<'T> -> Scaffold<'T> =
    fun rhNode ->
        ...
        actionBlock.Completion.ContinueWith(fun t -> linkableOutput.Complete()) |> ignore
        ...
        let targets, leaves, disposables =
            partCtors |> List.fold
                (fun state ctor ->
                    ...
                    newT :: targetsIn, newL |> List.append leavesIn,
                        newDs |> List.append disposablesIn
                )
                ([], List.empty<Task>, List.empty<IDisposable>) 
        let buffer = BufferBlock<'T>()
        buffer.Completion.ContinueWith(fun t -> targets |> List.iter (fun tgt -> tgt.Complete ())) |> ignore
